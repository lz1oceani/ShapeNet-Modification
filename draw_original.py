...
def dfs(dir_in, out_image, out_dae):
    ...
    for file in files:
        if len(file.split('.')) == 1:
            ...
            if file_name in visited and os.path.isfile(image_out):
                continue
            else:
            ...
            if file.split('.')[1] == 'dae':
                ...
                os.system('python process_dae_change_size.py -i ' + dae_in + ' -o ' + dae_out)
                ...
                os.system('python process_dae_change_size.py -i ./tmp/models/model.dae -o ./tmp/models/model.dae')
                os.system('blender278 -b -P render_dae.py -- -i ./tmp/models/model.dae -o ' + image_out)
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            print('python draw_all -n <number of files> -d <dir>')
            sys.exit()
        elif opt in ("-n", "--num"):
            number = int(arg)
        elif opt in ("-d", "--dir"):
    ...
    print(direct, outputdir + '_original_image/')
    dfs(direct + '/', outputdir + '_original_image/', outputdir + '_original_dae/')
